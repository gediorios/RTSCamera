...
namespace RTSCamera.Logic.SubLogic
{
    public class SwitchFreeCameraLogic
    {
        ...
        public List<FormationClass> PlayerFormations 
        {
            get 
            {
                if(_playerFormations == null)
                    _playerFormations = new List<FormationClass>();

                return _playerFormations;
            }
        }

        public FormationClass CurrentPlayerFormation
        {
            get => Mission.PlayerTeam?.TeamIndex < PlayerFormations.Count ? PlayerFormations[Mission.PlayerTeam.TeamIndex] : (FormationClass)_config.PlayerFormation;
            set
            {
        ...
        {
            Mission.OnMainAgentChanged -= OnMainAgentChanged;
            RTSEngineState.WatchMode = false;
        }

        public void OnFormationUnitsSpawned(Team team)
        {
            if (RTSEngineState.WatchMode && team == Mission.PlayerTeam && Mission.MainAgent == null)
            {
        ...
        public void OnAgentControllerChanged(Agent agent)
        {
            if (agent.Controller == Agent.ControllerType.Player || agent.Controller == Agent.ControllerType.None)
            {
                ...
                if (_config.AlwaysSetPlayerFormation && !RTSEngineState.WatchMode)
            ...
            if (Mission.MainAgent != null)
            {
                if (Mission.Mode == MissionMode.Battle || Mission.Mode == MissionMode.Deployment)
                {
                    if (_isFirstTimeMainAgentChanged)
                    {
                        ...
                        if (_config.UseFreeCameraByDefault || RTSEngineState.WatchMode)
                        {
        ...
        private void SwitchToAgent()
        {
            ...
            Utility.UpdateMainAgentControllerState(Mission.MainAgent, IsSpectatorCamera,
                _config.GetPlayerControllerInFreeCamera());

            MissionEvents.OnToggleFreeCamera(false);
        ...
        private void SwitchToFreeCamera()
        {
            ...
            {
                Utility.UpdateMainAgentControllerInFreeCamera(Mission.MainAgent, _config.GetPlayerControllerInFreeCamera());
                Utility.UpdateMainAgentControllerState(Mission.MainAgent, IsSpectatorCamera, _config.GetPlayerControllerInFreeCamera());
            }

            MissionEvents.OnToggleFreeCamera(true);
