    ...
    public class ControlTroopLogic
    {
        ...
        public bool SetToMainAgent(Agent agent)
        {
            if (agent != null)
            {
                ...
                {
                    MissionEvents.OnMainAgentWillBeChangedToAnotherOne(agent);
        ...
        public Agent GetAgentToControl()
        {
            ...
            {
                if ((!_switchFreeCameraLogic.IsSpectatorCamera || _flyCameraMissionView.LockToAgent) && _flyCameraMissionView.MissionScreen.LastFollowedAgent.Team == Mission.PlayerTeam) 
            ...
            {
                var firstPreference = AgentPreferenceFromFormation(_switchFreeCameraLogic.CurrentPlayerFormation, cameraPosition);
                ...
                if ((int)_switchFreeCameraLogic.CurrentPlayerFormation != _config.PlayerFormation)
                {
                    ...
                    return thirdPreference.companion ?? firstPreference.agent ?? secondPreference.agent ?? thirdPreference.agent;
                }
                else
                {
                    var thirdPreference = AgentPreferenceFromPlayerTeam(cameraPosition);
                    return thirdPreference.companion ?? firstPreference.agent ?? thirdPreference.agent;
                }
        ...
        private (Agent agent, Agent companion) AgentPreferenceFromFormation(FormationClass formationClass, Vec3 position)
        {
            var preference = new ControlAgentPreference();
